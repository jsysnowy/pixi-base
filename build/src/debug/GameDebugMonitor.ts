...
namespace com.sideplay.debug {
    export class DebugMonitor extends PIXI.Container {
         ...
         */
        public enable(): void {
            core.GameManager.instance.add(this);

            this._bottomBar = new PIXI.Graphics();
            this._bottomBar.beginFill(0x333333, 0.6);
            this._bottomBar.drawRect(0, config.settings.nativeHeight - 50, config.settings.nativeHeight, 50);
            this.addChild(this._bottomBar);

            this._fpsText = new PIXI.Text("FPS: ", { fill: 0xffffff });
            this._fpsText.position.set(40, config.settings.nativeHeight - 40);
        ...
        public onRaf(): void {
            if (this._storedDate != null) {
                ...
                if (this._updateUI == 5) {
                    ...
                    this._fpsGraph.drawRect(5, config.settings.nativeHeight - 155, 150, 100);
                    ...
                    for (var i = 0; i < this._avgFPS.length; i++) {
                        ...
                        this._fpsGraph.drawRect(5 + i, config.settings.nativeHeight - 55, 1, h);
                    }
                }
            }
